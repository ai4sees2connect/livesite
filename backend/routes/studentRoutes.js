...
router.post("/signup", async (req, res) => {
  ...
  try {
    ...
    const token = jwt.sign(
      { id: newStudent._id, userType: "Student" },
...
router.put("/api/:studentId/save-exp", async (req, res) => {
  ...
  try {
    ...
    return res
      ...
      .json({
        success: true,
        message: "yearsOfExp updated successfully.",
        student,
      });
  } catch (error) {
    console.error(error);
...
router.get("/resume/:id", async (req, res) => {
  try {
    ...
    res.setHeader(
      ...
      "Content-Disposition, Resume-Created-At",
    );
...
router.post("/batch-get-logos", async (req, res) => {
  try {
    ...
    recruiters.forEach((recruiter) => {
      ...
      logos[recruiter._id] =
        `/recruiter/internship/recruiter/logo/${recruiter._id}`;
    });
...
router.get("/:userId/internships", async (req, res) => {
  try {
    ...
    const appliedInternshipIds = student.appliedInternships.map(
      (app) => app.internship,
    );
