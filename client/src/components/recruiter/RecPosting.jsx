...
const RecPosting = () => {
    ...
    const fetchIndustries = async () => {
      try {
        const response = await axios.get(
          `${api}/recruiter/internship/industries`
        );
  ...
  // Fetch industry-specific skills
  const fetchIndustrySkills = async (industryName) => {
    try {
      const response = await axios.get(
        `${api}/recruiter/internship/industries/${industryName}/skills`
      );
      setIndustrySkills(response.data);
  ...
  const handleSubmit = async (e) => {
    ...
    // Stipend validation for paid internships
    if (formData.stipendType !== "unpaid" && !formData.stipend) {
    ...
    // Handle internship type and location
    ...
    try {
      ...
      toast.error(
        error.response?.data?.message || "Failed to post internship. Please try again."
      );
    }
  };
    ...
    return (
      <div className="flex flex-col justify-center items-center h-screen space-y-4">
        ...
        <Link
          ...
          className="px-4 py-2 bg-[var(--button-color)] text-white rounded-lg font-semibold hover:bg-[var(--button-hover-color)] transition-colors"
        >
          Back to profile
        </Link>
      </div>
    ...
    if (
      ...
      return (
        <div className="flex flex-col h-screen items-center justify-center space-y-3">
          <div className="text-center text-[var(--text-light)] text-lg font-semibold">
            You have used your monthly available free postings
          </div>
          <Link
            to={`/recruiter/${userId}/pricing`}
            className="w-fit mx-auto border bg-[var(--button-color)] px-4 py-2 font-semibold text-white rounded-lg hover:bg-[var(--button-hover-color)] transition-colors"
          >
            Upgrade your plan
          </Link>
        </div>
      ...
      return (
        <div className="flex flex-col space-y-3 h-screen">
          <div className="mt-[350px] text-center text-[var(--text-light)] text-lg font-semibold">
    ...
    <div className="min-h-screen bg-[var(--bg-light-color)] py-10 px-4">
      ...
      <form className="px-5">
        <div className="mx-auto w-full lg:w-[75%] bg-white rounded-[28px] border border-[var(--border-color)] p-8 md:p-10 shadow-[0_15px_60px_rgba(0,0,0,0.05)]">
            ...
            <div className="grid lg:grid-cols-[1fr_1fr_140px] gap-5">
              <select
                ...
                onChange={(e) => {
                  ...
                  if (value && value !== "other") {
                    fetchIndustrySkills(value);
                  }
                  setErrors((prev) => ({ ...prev, industry: "" }));
                }}
                className="h-[52px] w-full px-4 border border-[var(--border-color)] rounded-xl bg-white shadow-sm focus:outline-none focus:ring-4 focus:ring-[var(--icon-bg-color)] focus:border-[var(--primary-color)] text-[var(--text-color)]"
              >
              ...
              {selectedIndustry === "other" && (
                <input
                  ...
                  className="h-[52px] w-full px-4 border border-[var(--border-color)] rounded-xl shadow-sm focus:outline-none focus:ring-4 focus:ring-[var(--icon-bg-color)] focus:border-[var(--primary-color)] text-[var(--text-color)]"
                />
              )}

              {selectedIndustry !== "other" && <div></div>}
            </div>
          ...
          <div className="bg-[var(--bg-light-color)] p-6 rounded-2xl border border-[var(--border-color)] mb-6">
            <label className="text-xl font-bold text-[var(--text-color)] mb-3 block">
              Internship Title
            </label>
            <input
              type="text"
          ...
          <div className="my-8 bg-white border border-[var(--border-color)] rounded-3xl p-8 shadow-sm">
            <div className="flex items-center gap-3 mb-3">
              ...
              <h3 className="text-xl font-semibold text-[var(--text-color)]">
                Skills Required <span className="text-red-500 ml-1">*</span>
              </h3>
              {selectedIndustry && selectedIndustry !== "other" && (
                <span className="text-xs bg-[var(--icon-bg-color)] text-[var(--primary-color)] px-3 py-1 rounded-full ml-2">
                  {industrySkills.length} skills available
                </span>
              )}
            </div>
            ...
            <div className="grid lg:grid-cols-[1fr_1fr_140px] gap-5">
              <Select
                ...
                options={
                  industrySkills.length > 0 && selectedIndustry !== "other"
                ...
                placeholder={
                  selectedIndustry && selectedIndustry !== "other"
                    ? "Select skills from your industry"
                    : "Select or type skills"
                }
                className="w-full"
                styles={customSelectStyles}
                isDisabled={selectedIndustry === "other"}
              />
              ...
              <button
                ...
                onClick={() => {
                  if (selectedIndustry !== "other") {
                    toast.info(
                      "Please select 'Other' industry to add custom skills"
                    );
                    return;
                  }
                  addCustomSkill();
                ...
                className={`h-[52px] rounded-xl font-semibold shadow-md transition-all flex items-center justify-center gap-2 ${
                  selectedIndustry === "other"
            ...
            {selectedIndustry &&
              ...
              industrySkills.length === 0 && (
                <p className="text-yellow-600 mt-3 text-sm font-medium">
                  ⚠️ No skills available for this industry. Please select
                  "Other" to add custom skills.
                </p>
              )}
          </div>
          ...
          <div className="my-8">
            <p className="font-semibold text-lg mb-4 text-[var(--text-color)] flex items-center gap-2">
              <FaBriefcase className="text-[var(--primary-color)]" /> Internship
              Type
            </p>
                ...
                <label
                  ...
                  className={`cursor-pointer rounded-2xl border-2 p-5 transition-all ${
                    formData.internshipType === type
                      ? "border-[var(--primary-color)] bg-[var(--icon-bg-color)]"
                      : "border-[var(--border-color)] bg-white hover:border-[var(--primary-color)]/50"
                  }`}
                >
                  <input
                    type="radio"
                  ...
                  <div className="font-semibold text-[var(--text-color)] text-center">
                    {type}
                  </div>
            ...
            {(formData.internshipType === "Office" ||
              ...
              <div className="grid md:grid-cols-3 gap-4 mt-6">
                <select
                  className="h-12 rounded-xl border border-[var(--border-color)] px-4 shadow-sm focus:outline-none focus:ring-4 focus:ring-[var(--icon-bg-color)] focus:border-[var(--primary-color)] bg-white text-[var(--text-color)]"
                  id="country"
                  name="country"
                ...
                <select
                  className="h-12 rounded-xl border border-[var(--border-color)] px-4 shadow-sm focus:outline-none focus:ring-4 focus:ring-[var(--icon-bg-color)] focus:border-[var(--primary-color)] bg-white text-[var(--text-color)] disabled:bg-gray-100"
                  id="state"
                  value={selectedState}
                  onChange={(e) => {
          ...
          <div className="grid md:grid-cols-2 gap-6 my-6">
            <div className="flex flex-col">
              <label className="mb-3 text-lg font-semibold text-[var(--text-color)]">
                No of Openings
              </label>
              <input
                type="number"
                ...
                onChange={(e) => {
                  ...
                  if (
                    value === "" ||
                    (/^[1-9][0-9]?$/.test(value) && parseInt(value) <= 99)
                  )
                    handleChange(e);
                }}
                onKeyDown={(e) => {
                ...
                onBlur={handleBlur}
                className="h-12 px-4 border border-[var(--border-color)] rounded-xl shadow-sm focus:outline-none focus:ring-4 focus:ring-[var(--icon-bg-color)] focus:border-[var(--primary-color)] bg-white text-[var(--text-color)]"
            ...
            <div className="flex flex-col">
              ...
              <input
                ...
                onChange={(e) => {
                  ...
                  if (
                    value === "" ||
                    (/^[1-9][0-9]?$/.test(value) && parseInt(value) <= 24)
                  )
                    handleChange(e);
                }}
                onKeyDown={(e) => {
          ...
          <div className="my-8 bg-white border border-[var(--border-color)] rounded-3xl p-6 shadow-sm">
            ...
            <div className="bg-white rounded-2xl overflow-hidden border border-[var(--border-color)] shadow-sm">
              <ReactQuill
                ...
                onChange={(value) => {
                  if (value.length <= 3011) handleDescriptionChange(value);
                  else
                    setErrors((prev) => ({
                      ...prev,
                      description:
        ...
        <div className="my-10 mx-auto w-full lg:w-[75%] bg-white rounded-3xl border border-[var(--border-color)] shadow-sm overflow-hidden">
          <div className="px-5 py-4 border-b border-[var(--border-color)] bg-[var(--bg-light-color)]">
            <h3 className="text-xl font-bold text-[var(--text-color)] flex items-center gap-2">
              <FaFileAlt className="text-[var(--primary-color)]" /> Cover
              Letter, Availability & Assessment Question
            </h3>
            <p className="text-[var(--text-light)] text-sm mt-2">
              Cover letter and availability questions will be asked to every
              applicant by default. You may also add a customized assessment
              question.
            </p>
          </div>
          <div className="p-5">
            <div className="flex items-start gap-3 pb-4 border-b border-[var(--border-color)]">
              ...
              <div>
                <h4 className="font-semibold text-base text-[var(--text-color)]">
                  Cover Letter
                </h4>
                <p className="text-[var(--text-light)] text-sm mt-1">
                  Tell us something about yourself and why should you be hired
                  for this role.
                </p>
              </div>
            </div>
            ...
            <div className="pt-4">
              {!isAssessmentOpen ? (
                <button
                  ...
                  className="bg-[var(--button-color)] hover:bg-[var(--button-hover-color)] text-white px-5 py-2.5 rounded-xl font-semibold transition-all shadow-sm flex items-center gap-2"
                >
                ...
                <div>
                  <button
                    ...
                    className="text-red-500 font-semibold mb-3 flex items-center gap-2 hover:text-red-700 transition-colors"
                  >
                  ...
                  <textarea
                    ...
                    className="w-full p-3 border border-[var(--border-color)] rounded-xl focus:outline-none focus:ring-4 focus:ring-[var(--icon-bg-color)] focus:border-[var(--primary-color)] bg-white text-[var(--text-color)]"
                  />
          ...
          <div className="p-8">
            <div className="mb-10">
              ...
              <div className="flex flex-wrap gap-8 text-[17px] text-[var(--text-color)]">
                {["unpaid", "fixed", "negotiable", "performance-based"].map(
                  (type) => (
                    <label
                      key={type}
                    ...
                    >
                      <input
                        ...
                        name="stipendType"
                        value={type}
                        checked={formData.stipendType === type}
                        onChange={handleChange}
                        className="w-4 h-4 text-[var(--primary-color)] focus:ring-[var(--primary-color)]"
                      />
              ...
              {errors.stipendType && (
                <p className="text-red-500 mt-3">{errors.stipendType}</p>
              )}
              {(formData.stipendType === "fixed" ||
                ...
                <div className="flex items-center gap-3 mt-6">
                  ...
                  <input
                    ...
                    min="1"
                    className="h-12 w-44 px-4 border border-[var(--border-color)] rounded-xl shadow-sm focus:outline-none focus:ring-4 focus:ring-[var(--icon-bg-color)] focus:border-[var(--primary-color)] bg-white text-[var(--text-color)]"
                  />
              ...
              {formData.stipendType === "performance-based" && (
                <div className="mt-6">
                  ...
                  <textarea
                    ...
                    className="w-full p-4 border border-[var(--border-color)] rounded-xl shadow-sm focus:outline-none focus:ring-4 focus:ring-[var(--icon-bg-color)] focus:border-[var(--primary-color)] bg-white text-[var(--text-color)]"
                  />
            ...
            <div className="mb-10">
              <div className="flex items-center gap-3 mb-5">
                ...
                <label className="text-xl font-bold text-[var(--text-color)]">
                  Perks & Benefits
                </label>
              </div>
              <Select
                ...
                options={perks.map((perk) => ({ value: perk, label: perk }))}
                placeholder="Select perks"
                styles={customSelectStyles}
              />
              {errors.perksSet && (
                <p className="text-red-500 mt-2">{errors.perksSet}</p>
              )}
            </div>
            ...
            <div>
              ...
              {errors.ppoCheck && (
                <p className="text-red-500 mt-3">{errors.ppoCheck}</p>
              )}
            </div>
          </div>
        </div>
      ...
      <div className="w-full lg:w-[75%] mx-auto mt-10 mb-16 flex justify-center">
        <button
          ...
          className="px-12 py-3 rounded-xl bg-[var(--button-color)] text-white font-semibold shadow-md hover:bg-[var(--button-hover-color)] hover:shadow-lg transition-all flex items-center gap-2"
        >
