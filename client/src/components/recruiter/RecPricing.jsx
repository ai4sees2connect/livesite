  ...
  return (
    <div className="min-h-screen bg-[var(--bg-light-color)] py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-7xl mx-auto">
        ...
        {/* Pricing Cards Grid - Only 2 Plans Now */}
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-16 max-w-4xl mx-auto">
          ...
          <div className="bg-white rounded-3xl shadow-sm border border-[var(--border-color)] p-8 flex flex-col hover:shadow-md transition-shadow">
            ...
            <p className="text-[var(--text-light)] mt-1 text-sm">
            ...
            <ul className="space-y-2 mb-8 flex-1">
              ...
              <Feature included icon={<FaSearch />}>
                Basic Candidate Search
              </Feature>
              <Feature included icon={<FaHeadset />}>
                Standard Support
              </Feature>
          ...
          {/* 1 Year Plan - Updated */}
          <div className="relative bg-white rounded-3xl shadow-xl border-2 border-[var(--primary-color)] p-8 flex flex-col">
            ...
            <h3 className="text-2xl font-bold text-[var(--text-color)] mt-2">
              1 Year Plan
            </h3>
            <p className="text-[var(--text-light)] mt-1 text-sm">
              Ultimate growth for your company
            </p>
            <div className="mt-6 mb-6">
              <span className="text-4xl font-bold text-[var(--text-color)]">
                ₹4999
            ...
            <div className="bg-[var(--icon-bg-color)] p-4 rounded-xl mb-6 border border-[var(--border-color)]">
              <p className="text-sm font-bold text-[var(--primary-color)] flex items-center gap-2">
                <FaRocket /> 50 Job Postings / month
              </p>
              <p className="text-xs text-[var(--text-light)] mt-1">
                Total 600 postings annually
              </p>
            </div>
            <ul className="space-y-2 mb-8 flex-1">
              <Feature included icon={<FaRocket />}>
              ...
              <Feature included icon={<FaStar />}>
                Featured Job Listings
              </Feature>
              <Feature included icon={<FaChartLine />}>
                Advanced Analytics Dashboard
              </Feature>
              <Feature included icon={<FaPaintBrush />}>
                Custom Company Branding
              </Feature>
            </ul>

            {/* 1-Year Plan Button - Dynamic */}
            {recruiter.subscription.planType === "1-year" ? (
              <button className="w-full py-3 border-2 border-[var(--primary-color)] text-[var(--primary-color)] rounded-xl font-bold bg-[var(--icon-bg-color)] cursor-not-allowed">
                Current Plan
              </button>
            ) : (
              <button
                ...
                onClick={() => handlePayment(499900, "1-year")}
              >
                <FaBolt /> Upgrade Now
              </button>
            )}
        ...
        {/* Top-Up Section */}
        <div className="bg-white rounded-3xl shadow-sm border border-[var(--border-color)] p-8 mb-12 max-w-4xl mx-auto">
          <div className="flex flex-col md:flex-row md:items-center justify-between mb-6">
            <div>
              <h3 className="text-2xl font-bold text-[var(--text-color)] flex items-center gap-2">
                <FaPlus className="text-[var(--primary-color)]" /> Need more
                postings right now?
              </h3>
              <p className="text-[var(--text-light)] mt-1">
                Buy top-up posts instantly. No subscription required.
              </p>
            </div>
          </div>
        ...
        <div className="bg-white rounded-3xl shadow-sm border border-[var(--border-color)] p-8 max-w-4xl mx-auto">
          <h3 className="text-xl font-bold text-[var(--text-color)] mb-6 flex items-center gap-2">
            <FaInfoCircle className="text-[var(--primary-color)]" /> Current
            Subscription Status
          </h3>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="bg-[var(--bg-light-color)] p-5 rounded-2xl border border-[var(--border-color)]">
              ...
              <p className="text-xl font-bold text-[var(--primary-color)] capitalize">
                {planType}
              </p>
            </div>
            <div className="bg-[var(--bg-light-color)] p-5 rounded-2xl border border-[var(--border-color)]">
            ...
            <div className="bg-[var(--bg-light-color)] p-5 rounded-2xl border border-[var(--border-color)]">
              ...
              {recruiter.subscription.planType !== "free" ? (
                <p className="text-lg font-bold text-red-500">
                  Expires in {TimeLeft(recruiter.subscription.expirationDate)}{" "}
                  days
                </p>
              ) : (
                <p className="text-lg font-bold text-green-600">
                  ...
                  {calculateDaysRemaining(
                    recruiter.subscription.activationDate,
                  )}{" "}
                  days
                </p>
              )}
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex justify-center items-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-2xl p-8 max-w-md w-full text-center">
            ...
            <p className="text-[var(--text-light)] mb-6">
              You already have an active plan. Do you want to merge this new
