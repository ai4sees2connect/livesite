...
const SubmitAssignment = ({ isOpen, onClose, onSubmit, msgId }) => {
  ...
  const handleFileUpload = async (event) => {
    ...
    const totalSize = [...updatedFiles, ...newFiles].reduce(
  ...
  return (
    <div className="fixed inset-0 bg-opacity-20 bg-gray-300  flex justify-center items-center z-50 mx-3 lg:mx-0">
      <div className="bg-white rounded-lg shadow-lg p-6 max-w-[670px]">
        ...
        <div className="mb-4 ">
          ...
          <div
            ...
            className="flex items-center justify-center space-x-3 border border-dashed bg-blue-100  border-blue-400 cursor-pointer w-[200px] h-[90px] "
          >
          ...
          <div className="mt-4 flex flex-wrap space-x-3 text-sm ">
            {uploadedFiles.length > 0 &&
              uploadedFiles.map((file, index) => (
                <div
                  key={index}
                  className="flex text-gray-600 space-x-2 border items-center w-fit justify-between  p-2 rounded-md mb-2"
                >
        ...
        {/* Assignment Link */}
        <div className="mb-4">
          <label className="items-center mb-2 inline-flex">
            Assignment Link{" "}
            <FaPaperclip className="mx-1 text-gray-700 w-3 h-3" />
          </label>
          <input
            type="url"
